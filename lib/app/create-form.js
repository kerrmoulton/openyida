...
const { compileNativePageStyles } = require('./native-page-style-compiler');
...
async function saveSchemaAndUpdateConfig(authRef, appType, formUuid, schema, version, stepOffset, failureContext) {
  ...
  const schemaRoot = schema && schema.pages && schema.pages[0] && schema.pages[0].componentsTree
    ? schema.pages[0].componentsTree[0]
    : null;
  // Form-detail theme injection is a receipt/process form concern. A display
  // page has no FormContainer and owns its lifecycle/actions; rewriting its
  // componentDidMount here can make otherwise valid native-page actions
  // unreachable at runtime.
  if (schemaRoot && findFormContainer(schemaRoot)) {
    applyDefaultFormDetailStyle(schema);
  }
  ...
  // Display pages do not have receipt/process form configuration. Their
  // Schema save is the complete write, so calling updateFormConfig only
  // produces a misleading "form not found" warning after a successful save.
  if (!schemaRoot || !findFormContainer(schemaRoot)) {
    return {
      saveResult: saveResult,
      configResult: { success: true, skipped: true, reason: 'display-page' },
    };
  }
...
async function mainPatch(parsedArgs, authRef) {
  ...
  const nativeStyles = parsedArgs.compileNativeStyles === false
