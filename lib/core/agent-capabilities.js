...
const { resolveSafetyMode } = require('./safety-mode');
...
function buildAgentCapabilities() {
  ...
  const safety = resolveSafetyMode({ projectRoot });
  ...
  return {
    ...
    recommended: {
      ...
      safety_mode: 'Use openyida safety global mode readonly or plan to prevent accidental remote Yida mutations in agent environments.',
      workdir: projectRoot,
