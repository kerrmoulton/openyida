...
function getAuthRef() {
  ...
  if (!cookieData || !cookieData.cookies || !cookieData.cookies.length) {
    cookieData = triggerLogin();
  }

  const cookieInfo = extractInfoFromCookies((cookieData && cookieData.cookies) || []);
  const csrfToken = (cookieData && cookieData.csrf_token) || cookieInfo.csrfToken;
  if (!cookieData || !cookieData.cookies || !csrfToken) {
